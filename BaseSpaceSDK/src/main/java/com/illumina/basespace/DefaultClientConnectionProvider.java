...
class DefaultClientConnectionProvider implements ClientConnectionProvider
{
    ...
    private Client createClient()
    {
        ...
        client.addFilter(new ClientFilter()
        {
            ...
            {
                logger.fine("[QGMO FIlter] " + request.getMethod() + " to " + request.getURI().toString());
                if (accessToken != null)
                {
                    request.getHeaders().add("x-access-token", accessToken);
                }
                
                logger.fine("[QGMO] request context added attribute x-access-token : " + accessToken + "    hd= " +request.getHeaders());
                ...
                try
                {
                    ...
                    logger.fine("[QGMO] response headers = " + response.getHeaders());
